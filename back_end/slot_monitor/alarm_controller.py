# ============================================================
# FILE: back_end/slot_monitor/alarm_controller.py
# ============================================================
"""
Centralized alarm state management.

Emit paths (in priority order):
  1. Redis publisher  — set via set_redis_publisher(); used in multi-process mode
  2. SocketIO         — set via set_socketio(); used in single-process mode
  3. ntfy push        — set via set_ntfy_push(); optional LAN push notifications
                        runs fire-and-forget alongside path 1 or 2

Sound lifecycle:
    trigger()    → starts sound + emits alarm_triggered
    silence()    → stops sound only, mismatches remain
    unsilence()  → restarts sound if mismatches still exist
    resolve()    → removes one mismatch; stops sound if set empties
    clear()      → admin override — clears all mismatches + stops sound

Thread-safety:
    All emit calls happen OUTSIDE the internal lock to avoid blocking
    monitor workers on network I/O.

    A per-(pid,lid) timestamp debounce prevents BackgroundEncoder from
    being flooded when a slot triggers rapidly.
"""

import logging
import threading
import time

from back_end.secrets import Secrets
from back_end.config import AlarmConfig as _AC

logger = logging.getLogger(__name__)

_CLIP_DEBOUNCE_S = _AC.CLIP_DEBOUNCE_S


class AlarmController:

    def __init__(self):
        self._lock             = threading.Lock()
        self.active            = False
        self.mismatches: set[tuple[str, int]] = set()
        self._alarm_start_time = None
        self._silenced         = False

        # Emit targets — set after construction
        self.socketio          = None    # direct SocketIO (single-process)
        self._redis_pub        = None    # Redis publisher (multi-process)
        self._ntfy:            object = None  # NtfyPush instance (optional LAN push)

        # Debounce: avoid flooding BackgroundEncoder with alarm clips
        self._last_clip_time: dict[tuple[str, int], float] = {}

    # ── Emit target setters ───────────────────────────────────────────────────

    def set_socketio(self, socketio) -> None:
        with self._lock:
            self.socketio = socketio

    def set_redis_publisher(self, publisher) -> None:
        """
        Set a Redis publisher for multi-instance alarm fanout (Opt #39).
        Pass None to revert to direct SocketIO emit.
        """
        with self._lock:
            self._redis_pub = publisher
        if publisher is not None:
            logger.info("[AlarmController] Redis pub/sub fanout enabled (Opt #39)")
        else:
            logger.info("[AlarmController] Redis pub/sub disabled — direct SocketIO emit")

    def set_ntfy_push(self, ntfy_instance) -> None:
        """
        Set a NtfyPush instance for LAN push notifications (Opt #34 LAN variant).
        Fire-and-forget alongside the primary SocketIO/Redis path.
        """
        with self._lock:
            self._ntfy = ntfy_instance
        if ntfy_instance is not None and getattr(ntfy_instance, "enabled", False):
            logger.info("[AlarmController] ntfy LAN push enabled")
        else:
            logger.info("[AlarmController] ntfy push disabled or not configured")

    # ── Sound control ─────────────────────────────────────────────────────────

    def silence(self) -> None:
        with self._lock:
            if self.active and not self._silenced:
                self._stop_alarm_sound()
                self._silenced = True
                logger.info("Alarm silenced — mismatches remain, resolution in progress")

    def unsilence(self) -> None:
        with self._lock:
            if self.active and self.mismatches and self._silenced:
                self._silenced = False
                self._start_alarm_sound()
                logger.warning("Alarm unsilenced — admin quit resolution without finishing")

    # ── Mismatch tracking ─────────────────────────────────────────────────────

    def trigger(self, pid: str, lid: int) -> None:
        """
        Register a new mismatch and (if needed) activate the alarm.

        All SocketIO/Redis/ntfy calls happen OUTSIDE the lock so that the
        alarm lock never blocks monitor workers waiting on network I/O.
        """
        pid = str(pid)

        first_trigger = False
        snapshot      = None
        sio           = None
        redis_pub     = None
        ntfy          = None

        with self._lock:
            sio       = self.socketio
            redis_pub = self._redis_pub
            ntfy      = self._ntfy

            if not self.active:
                self._silenced         = False
                self._alarm_start_time = time.time()
                self.active            = True
                self._start_alarm_sound()
                logger.warning("ALARM ACTIVATED")
                first_trigger = True

            self.mismatches.add((pid, lid))
            logger.warning(f"Mismatch added: PID={pid}, LID={lid}")
            snapshot = list(self.mismatches)

        # ── Emit OUTSIDE lock ──────────────────────────────────────────────
        if snapshot is not None:
            mismatch_list = [[p, l] for p, l in snapshot]

            if redis_pub is not None:
                if first_trigger:
                    redis_pub.publish("alarm_triggered", {
                        "mismatch_count": len(snapshot),
                        "mismatches":     mismatch_list,
                    })
                redis_pub.publish("alarm_updated", {
                    "mismatch_count": len(snapshot),
                    "mismatches":     mismatch_list,
                })
            elif sio is not None:
                if first_trigger:
                    sio.emit("alarm_triggered", {
                        "mismatch_count": len(snapshot),
                        "mismatches":     mismatch_list,
                    })
                sio.emit("alarm_updated", {
                    "mismatch_count": len(snapshot),
                    "mismatches":     mismatch_list,
                })

            # ntfy is always fire-and-forget alongside the primary path
            if first_trigger and ntfy is not None:
                try:
                    ntfy.send_alarm(
                        mismatch_count = len(snapshot),
                        mismatches     = mismatch_list,
                    )
                except Exception as e:
                    logger.debug(f"[AlarmController] ntfy send failed (non-critical): {e}")

        self._save_alarm_clips(pid, lid)

    def resolve(self, pid: str, lid: int) -> None:
        sio       = None
        redis_pub = None
        ntfy      = None
        cleared   = False

        with self._lock:
            self.mismatches.discard((pid, lid))
            logger.info(f"Mismatch resolved: PID={pid}, LID={lid}")
            if self.active and not self.mismatches:
                self._clear_active_alarm()
                cleared = True
            sio       = self.socketio
            redis_pub = self._redis_pub
            ntfy      = self._ntfy

        if cleared:
            if redis_pub is not None:
                redis_pub.publish("alarm_cleared", {})
            elif sio is not None:
                sio.emit("alarm_cleared", {})

            if ntfy is not None:
                try:
                    ntfy.send_alarm_cleared()
                except Exception:
                    pass

    def stop_if_clear(self) -> None:
        sio       = None
        redis_pub = None
        ntfy      = None
        cleared   = False

        with self._lock:
            if self.active and not self.mismatches:
                self._clear_active_alarm()
                cleared = True
            sio       = self.socketio
            redis_pub = self._redis_pub
            ntfy      = self._ntfy

        if cleared:
            if redis_pub is not None:
                redis_pub.publish("alarm_cleared", {})
            elif sio is not None:
                sio.emit("alarm_cleared", {})
            if ntfy is not None:
                try:
                    ntfy.send_alarm_cleared()
                except Exception:
                    pass

    def clear(self) -> None:
        sio       = None
        redis_pub = None
        ntfy      = None
        cleared   = False

        with self._lock:
            count = len(self.mismatches)
            self.mismatches.clear()
            if self.active:
                self._clear_active_alarm()
                cleared = True
            logger.info(f"Alarm force-cleared ({count} mismatches removed)")
            sio       = self.socketio
            redis_pub = self._redis_pub
            ntfy      = self._ntfy

        if cleared:
            if redis_pub is not None:
                redis_pub.publish("alarm_cleared", {})
            elif sio is not None:
                sio.emit("alarm_cleared", {})
            if ntfy is not None:
                try:
                    ntfy.send_alarm_cleared()
                except Exception:
                    pass

    # ── Private helpers ───────────────────────────────────────────────────────

    def _clear_active_alarm(self) -> None:
        """Must be called with self._lock held. Caller emits alarm_cleared."""
        if not self._silenced:
            self._stop_alarm_sound()
        duration               = time.time() - self._alarm_start_time if self._alarm_start_time else 0
        self.active            = False
        self._silenced         = False
        self._alarm_start_time = None
        logger.info(f"ALARM CLEARED after {duration:.1f}s")

    def _save_alarm_clips(self, pid: str, lid: int) -> None:
        """
        Submit rolling-buffer snapshots to BackgroundEncoder — non-blocking.
        Debounced per (pid, lid) to avoid flooding the encoder queue.
        """
        key = (pid, lid)
        now = time.time()
        if now - self._last_clip_time.get(key, 0.0) < _CLIP_DEBOUNCE_S:
            logger.debug(f"[Alarm] Clip save debounced for PID={pid} LID={lid}")
            return
        self._last_clip_time[key] = now

        try:
            from back_end.slot_monitor.camera.rolling_buffer import (
                face_rolling_buffer, top_rolling_buffer,
            )
            face_rolling_buffer.save_alarm_clip(pid=pid, lid=lid)
            top_rolling_buffer.save_alarm_clip(pid=pid, lid=lid)
        except Exception as e:
            logger.warning(f"[Alarm] Failed to queue alarm clips: {e}")

    # ── Admin auth ────────────────────────────────────────────────────────────

    def authenticate_admin(self, password: str) -> dict:
        if password == Secrets.ADMIN_PASSWORD:
            with self._lock:
                snapshot = list(self.mismatches)
            return {
                "authenticated": True,
                "mismatches":    sorted([(str(p), l) for p, l in snapshot]),
            }
        return {"authenticated": False, "mismatches": []}

    def get_status(self) -> dict:
        with self._lock:
            return {
                "active":         self.active,
                "silenced":       self._silenced,
                "mismatch_count": len(self.mismatches),
                "duration":       (
                    time.time() - self._alarm_start_time
                    if self._alarm_start_time else 0
                ),
            }

    # ── Hardware interface ────────────────────────────────────────────────────

    def _start_alarm_sound(self) -> None:
        print("ALARM ON")
        logger.warning("Physical alarm started")

    def _stop_alarm_sound(self) -> None:
        print("ALARM OFF")
        logger.info("Physical alarm stopped")
