...
def create_phone(data):
    ...
    try:
        with conn.cursor() as cur:
            ...
            pid = cur.fetchone()[0]
            conn.commit()
            return {"status": "success", "pid": pid}, 201
...
def list_phones():
    ...
    try:
        with conn.cursor() as cur:
            ...
            rows = cur.fetchall()
            columns = [desc[0] for desc in cur.description]
            return [dict(zip(columns, r)) for r in rows], 200
    finally:
        put_conn(conn)
    ...
    try:
        with conn.cursor() as cur:
            ...
            if cur.rowcount == 0:
                return {"error": "Phone not found"}, 404
            conn.commit()
            return {"status": "success", "sid": sid}, 200
    except Exception as e:
        conn.rollback()
        return {"status": "error", "error": str(e)}, 400
    finally:
    ...
    try:
        with conn.cursor() as cur:
            ...
            if cur.rowcount == 0:
                return {"error": "Phone not found"}, 404
            conn.commit()
            return {"status": "deleted", "sid": sid}, 200
    finally:
        put_conn(conn)
...
def phones_by_condition(cond):
    ...
    try:
        with conn.cursor() as cur:
            ...
            rows = cur.fetchall()
            if not rows:
                return {"message": f"No phones with condition '{cond}'"}, 404
            columns = [desc[0] for desc in cur.description]
            return [dict(zip(columns, r)) for r in rows], 200
    finally:
        put_conn(conn)
...
def phone_stats():
    ...
    try:
        with conn.cursor() as cur:
            ...
            result = cur.fetchone()
            columns = [desc[0] for desc in cur.description]
            return dict(zip(columns, result)), 200
    finally:
        put_conn(conn)


def reassign_phone(data):
    old_sid = data.get("old_sid")
    new_sid = data.get("new_sid")

    if not old_sid or not new_sid:
        return {"error": "old_sid and new_sid are required"}, 400
    ...
    try:
        with conn.cursor() as cur:
            ...
            if cur.rowcount == 0:
                return {"error": "Phone not found for the given old_sid"}, 404
            conn.commit()
            return {"status": "success", "new_owner": new_sid}, 200
    except Exception as e:
        conn.rollback()
        return {"status": "error", "error": str(e)}, 400
    finally:
    ...
    try:
        with conn.cursor() as cur:
            ...
            if cur.rowcount == 0:
                return {"error": "Phone not found"}, 404
            conn.commit()
            return {"status": "PID regenerated", "sid": sid}, 200
    finally:
        put_conn(conn)
