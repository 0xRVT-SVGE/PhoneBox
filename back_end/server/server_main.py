...
def handle_toggle_scan(_):
    ...
    if new_state:
        # Starting a scan - initialize state and trigger start event
        ...
        start_scan()  # Set the start event (0% CPU operation)
    else:
        # Stopping a scan - set stop event and unblock queue
        stop_scan()
...
if __name__ == "__main__":
    # Start the persistent worker thread ONCE at startup
    worker_thread = threading.Thread(target=scan_worker, daemon=True, name="ScanWorker").start()
    threading.Thread(target=_start_async_loop, args=(async_loop,), daemon=True).start()
    threading.Thread(target=lambda: scanner_loop(debugwindow, debugroi), daemon=True).start()
