...
class _AdminResolutionPageState extends State<AdminResolutionPage>
  ...
  /// Backend lids are 0-based; UI always shows 1-based.
  static int _displaySlot(int? lid) => (lid ?? 0) + 1;

  /// Formats a PID for the UI.
  static String _displayPid(String? pid) {
    if (pid == null) return '?';
    if (pid.startsWith('unknown-')) return 'Unknown object';
    if (pid.length > 20) return '…${pid.substring(pid.length - 12)}';
    return pid;
  }
  ...
  Future<void> _initVideo() async {
    ...
    if (widget.inheritedPeerConnection != null) {
      ...
      // Attempt to recover already-flowing track
      try {
  ...
  void _waitForServer() {
    ...
    _serverTimer = Timer(const Duration(seconds: 10), () {
      ...
      setState(() {
        ...
        _scanError = 'No response from server. Check your connection and try again.';
      });
    });
  }
  ...
  void _registerCallbacks() {
    _socket.connect(
      ...
      onAdminQrResult: (data) {
        ...
        final bool needsDep = data['needs_deposit'] == true;
        final bool occupied = data['target_occupied'] == true;
        _currentPid = pid;
  ...
  String _friendlyError(String code) {
    const map = {
      ...
      'step_lock_violated':
      'Place or stage the current phone before picking up another.',
  ...
  bool get _canDeclareCurrentMissing {
    ...
    final currentPid = _remaining.firstWhere(
            (p) => _mismatchMap[p] == _currentLid,
        orElse: () => '');
    if (currentPid.isEmpty) return false;
    final otherRemaining =
    _remaining.where((p) => p != currentPid).toSet();
    return otherRemaining.every((p) => _visitedPids.contains(p));
  }
  ...
  void _onPlaced() {
    if (_expectedLid == null) {
      setState(() => _scanError =
      'Cannot place: expected slot is unknown. Tap "Handle a different phone" and try again.');
      return;
    }
    _waitForServer();
  ...
  Future<void> _onDeclareMissing() async {
    final pid = _remaining.isNotEmpty ? _remaining.first : null;
    if (pid == null) return;
    setState(() => _step = _Step.declaringMissing);
    final confirm = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        title: const Text('Declare phone missing?'),
        content: Text(
            '${_displayPid(pid)} was never physically found.\n'
                'This will withdraw its DB record and permanently record evidence.'),
        actions: [
          TextButton(
              onPressed: () => Navigator.pop(context, false),
              child: const Text('Cancel')),
    ...
    if (confirm == true) {
      ...
      setState(() => _step = _Step.selectingPhone);
    }
  }
      ...
      onPopInvokedWithResult: (didPop, _) {
        if (!didPop) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content:
            Text('Use the button below to close the session properly.'),
  ...
  Widget _buildCamera() {
    return Positioned.fill(
      child: _videoConnected && _renderer.srcObject != null
      ...
      child: SafeArea(
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              Row(children: [
                if (_step == _Step.awaitingScan ||
                    _step == _Step.scanning)
                  _topBadge(
                      Icons.fiber_manual_record, 'REC', Colors.red),
                const SizedBox(width: 8),
                _topBadge(
                    Icons.videocam_outlined, 'TOP CAM', Colors.white24),
                const Spacer(),
                if (total > 0)
                  Text('$done / $total',
    ...
    switch (_step) {
      ...
      case _Step.pickingUp:
        return _stepContent(
          ...
          color: Colors.orange,
          title:
          'Pick up the object from slot ${_displaySlot(_currentLid)}',
          subtitle:
          'Once you have it in hand, tap the button below.',
          error: _scanError,
          actions: [
            _primaryBtn(
                "I've picked it up", Colors.orange, _onPickedUp),
            _changePhoneBtn(),
            const SizedBox(height: 4),
            Opacity(
              opacity: (_canDeclareCurrentMissing && !_pendingServer)
                  ? 1.0
                  : 0.35,
              child: TextButton.icon(
                ...
                onPressed:
                (_canDeclareCurrentMissing && !_pendingServer)
      ...
      case _Step.placing:
        return _stepContent(
          ...
          title:
          'Place ${_displayPid(_currentPid)} in slot ${_displaySlot(_expectedLid)}',
          subtitle:
          'Slide it in without covering it. Tap Done when it\'s in.',
          error: _scanError,
          actions: [
            _primaryBtn(
                'Done — placed in slot ${_displaySlot(_expectedLid)}',
                Colors.green,
                _onPlaced),
          ],
        );
      case _Step.stagingNeeded:
        return _stepContent(
          ...
          color: Colors.amber,
          title:
          'Target slot ${_displaySlot(_expectedLid)} is occupied — stage this phone',
          subtitle:
          ...
          actions: [
            _primaryBtn(
                'I\'ve placed it in staging', Colors.amber, _onStaged),
          ],
        );
      case _Step.unstageNext:
        ...
        return _stepContent(
          ...
          subtitle:
          'Pick it up from the staging zone, then place it in slot ${lid + 1}.',
          error: _scanError,
          actions: [
            _primaryBtn(
                'I\'ve retrieved it', Colors.teal, _onUnstageNext),
          ],
        );
      case _Step.noQrHandled:
        return _stepContent(
          ...
          error: _scanError,
          actions: [
            _primaryBtn(_nextLabel(), null, _onNextAfterInfo)
          ],
        );
      case _Step.needsDeposit:
        return _stepContent(
          ...
          subtitle:
          _infoMessage ?? 'This phone has no storage record.',
          error: _scanError,
          actions: [
            _primaryBtn(_nextLabel(), null, _onNextAfterInfo)
          ],
  ...
  Widget _openingContent() {
    if (_openingTimedOut) {
      return Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(height: 8),
        const Text(
          'Check that migration 004 is applied and admin handlers are registered.',
    ...
    return Column(
      ...
      children: [
        ...
        if (_remaining.isNotEmpty) ...[
          ...
          ..._remaining.map((pid) {
            final isDefault =
                pid == _remaining.first && !hasStaged;
            return _phoneChip(
              pid: pid,
              lid: _mismatchMap[pid] ?? 0,
              color: isDefault
  ...
  Widget _doneContent() {
    final resolved =
        (_summary?['resolved'] as List?)?.length ?? 0;
    final missing =
        (_summary?['declared_missing'] as List?)?.length ?? 0;
    final depositing =
        (_summary?['needs_deposit'] as List?)?.length ?? 0;
    return Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      Wrap(
          ...
          children: [
            ...
            if (missing > 0)
              _summaryChip('$missing missing', Colors.red),
            if (depositing > 0)
      ...
      _primaryBtn(
          'Close', Colors.green, () => Navigator.of(context).pop(true)),
    ]);
  }
